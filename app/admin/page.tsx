...
export default function AdminPage() {
  ...
  const fetchPedidos = async () => {
    ...
    try {
      ...
      setError("No se pudieron cargar los pedidos. Revisa las reglas de seguridad de Firestore.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gray-100">
      <header className="bg-white shadow-md p-4 flex justify-between items-center sticky top-0 z-10">
        ...
        <div className="flex items-center space-x-4">
          ...
          {!loading && pedidos.length > 0 && (
            <button onClick={() => setShowDeleteAllModal(true)} className="bg-red-500 text-white font-semibold py-2 px-4 rounded-lg hover:bg-red-600">
              {t('deleteAll')}
            </button>
          )}
      ...
      <main className="p-4 md:p-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-gradient-to-br from-blue-400 to-blue-600 text-white p-6 rounded-xl shadow-lg">
            <h2 className="font-semibold text-lg">{t('pendingOrders', { defaultValue: 'Pedidos Pendientes' })}</h2>
          ...
          <div className="bg-gradient-to-br from-orange-400 to-orange-600 text-white p-6 rounded-xl shadow-lg">
            <h2 className="font-semibold text-lg">{t('pendingEggBoxes', { defaultValue: 'Cajas Huevos Pend.' })}</h2>
          ...
          <div className="bg-gradient-to-br from-green-400 to-green-600 text-white p-6 rounded-xl shadow-lg">
            <h2 className="font-semibold text-lg">{t('pendingOilCans', { defaultValue: 'Garrafas Aceite Pend.' })}</h2>
        ...
        <div className="mt-8">
          ...
          {!loading && !error && pedidos.length === 0 && (
            <div className="bg-white rounded-lg shadow p-8 text-center text-gray-500">
              ...
              <h2 className="mt-4 text-2xl font-semibold">{t('noOrdersYet')}</h2>
              <p className="mt-2">{t('newOrdersAppearHere')}</p>
            </div>
          )}
          {!loading && !error && pedidos.length > 0 && (
                ...
                <div key={pedido.id} className={`bg-white rounded-lg shadow-md p-4 transition-opacity duration-300 ${pedido.estado === 'entregado' ? 'opacity-50' : ''}`}>
                  <div className="flex justify-between items-start border-b pb-2 mb-2">
                    <div className="flex items-start space-x-3">
                      <input type="checkbox" checked={pedido.estado === 'entregado'} onChange={() => handleToggleEstado(pedido.id, pedido.estado)} className="mt-1 h-6 w-6 rounded border-gray-300 text-green-600 focus:ring-green-500 cursor-pointer" />
                      <div>
                    ...
                    <div className="flex items-center space-x-2">
                      <button onClick={() => handleTogglePagado(pedido.id, pedido.pagado)} className={`py-1 px-3 rounded-full text-sm font-semibold ${pedido.pagado ? 'bg-green-100 text-green-800 hover:bg-green-200' : 'bg-orange-100 text-orange-800 hover:bg-orange-200'}`}>
                        {pedido.pagado ? `✅ ${t('paid')}` : `💳 ${t('pending')}`}
                      </button>
                      <button onClick={() => setPedidoParaEliminar(pedido)} className="text-red-500 hover:text-red-700 p-2 rounded-full hover:bg-red-100" title="Eliminar pedido">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm4 0a1 1 0 012 0v6a1 1 0 11-2 0V8z" clipRule="evenodd" /></svg>
                      </button>
                    </div>
                  </div>
                  <div className="space-y-2">
                    {pedido.productos.huevos && <div className="bg-orange-50 p-2 rounded-md flex justify-between items-center"><div><span className="text-2xl mr-2">🥚</span><span>{pedido.productos.huevos.cantidad} {t('boxUnit', { count: pedido.productos.huevos.cantidad })}s ({pedido.productos.huevos.unidades} uds)</span></div><span className="font-bold">{pedido.productos.huevos.precioTotal}€</span></div>}
                    {pedido.productos.aceite && <div className="bg-green-50 p-2 rounded-md flex justify-between items-center"><div><span className="text-2xl mr-2">🍾</span><span>{pedido.productos.aceite.cantidad} {t('canUnit', { count: pedido.productos.aceite.cantidad })}s ({pedido.productos.aceite.litros} L)</span></div><span className="font-bold">{pedido.productos.aceite.precioTotal}€</span></div>}
                  </div>
      ...
      <Modal isOpen={showDeleteAllModal} onClose={() => setShowDeleteAllModal(false)}>
        <div className="space-y-4">
          ...
          <p className="text-gray-600">Esta acción eliminará permanentemente <strong>TODOS</strong> los pedidos. No podrás recuperarlos.</p>
