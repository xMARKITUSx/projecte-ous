              ...
              {pedidos.map(pedido => (
                <div key={pedido.id} className={`bg-white rounded-lg shadow-md p-4 transition-opacity duration-300 ${pedido.estado === 'entregado' ? 'opacity-50' : ''}`}>
                  <div className="flex justify-between items-start border-b pb-2 mb-2">
                    <div className="flex items-start space-x-3"><input type="checkbox" checked={pedido.estado === 'entregado'} onChange={() => handleToggleEstado(pedido.id, pedido.estado)} className="mt-1 h-6 w-6 rounded border-gray-300 text-green-600 focus:ring-green-500 cursor-pointer" /><div><p className={`font-bold text-lg text-gray-900 ${pedido.estado === 'entregado' ? 'line-through' : ''}`}>{pedido.cliente}</p><p className="text-sm text-gray-500">{pedido.telefono}</p></div></div>
                    <div className="flex items-center space-x-2">
                      <button onClick={() => handleTogglePagado(pedido.id, pedido.pagado)} className={`py-1 px-3 rounded-full text-sm font-semibold ${pedido.pagado ? 'bg-green-100 text-green-800 hover:bg-green-200' : 'bg-orange-100 text-orange-800 hover:bg-orange-200'}`}>{pedido.pagado ? `✅ ${t('paid')}` : `💳 ${t('pending')}`}</button>
                      <button onClick={() => setPedidoParaEliminar(pedido)} className="text-red-500 hover:text-red-700 p-2 rounded-full hover:bg-red-100" title="Eliminar pedido"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm4 0a1 1 0 012 0v6a1 1 0 11-2 0V8z" clipRule="evenodd" /></svg></button>
                    </div>
                  </div>
